...
def add_to_tuple(tup, num):
    return tup + (num,)

def all_divisible(tup, num):
    if tup:
        remainder = num % tup[-1]
        return remainder == 0
    else:
...
def add_to_subsets(i, subsets):
    new_subsets = []
    for subset in subsets:
        if all_divisible(subset, i):
            new_subs = add_to_tuple(subset, i)
            new_subsets.append(new_subs)
    subsets += new_subsets
...
class SlowSolution:
    def largestDivisibleSubset(self, nums: List[int]) -> List[int]:
        ...
        for i in nums:
            add_to_subsets(i, subsets)

        return list(max(subsets, key=len))
...
# result = Solution().largestDivisibleSubset(list_in)
...
# For this solution
# create a list containing a singleton set of the first number in nums
#   for each following number, go back through the list and find the largest set that the number fits into
#   append this set to the list
#   return the largest of the sets
# todo fix this, still does not work
def fit_together(num, num_set):
    return num % num_set[-1] == 0
...
class Solution:
    def largestDivisibleSubset(self, nums: List[int]) -> List[int]:
        ...
        div_sets = [[sorted_nums[0]]]
        for number in sorted_nums[1:]:
            largest_fitting_subset = []
            for d_set in div_sets:
                if fit_together(number, d_set):
                    largest_fitting_subset = max(largest_fitting_subset, d_set, key=len)
            new_subset = largest_fitting_subset + [number]
            div_sets.append(new_subset)
        
        return list(max(div_sets, key=len))
